...
site_params = {1: {'site_name':'ES-LM1', 'lat': 39.945592, 'lon':-5.782977, 'stdlon':-15.0, 'z_u': 15, 'z_t': 15,
...
for year in years:
    ...
    for site in sites:
        ...
        #grass vegetation depends on LAI (like Sen-ET, Guzinski et al. 2020)
        hc_factor = np.minimum((LAI_grass/2.)**3, 1)
        hC_sub = 0.2*hC_sub_value + 0.8*(hC_sub_value*hc_factor)
        ...
        #plot half-hourly modelled vs observed flux scatter plot
        ...
        plt.savefig(join(outdir_site, 'FluxPartitioning_%s_%s_%s_Site%s.png' % (res_string, G_string, year, site )))
        plt.close()
        ...
        plt.savefig(join(outdir_site, 'AvailableEnergy_vs_obs_%s_%s_%s_Site%s.png' % (res_string, G_string, year, site)))
        plt.close()
