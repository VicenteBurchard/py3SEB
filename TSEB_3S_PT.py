...
# ==============================================================================
# Function for threeSEB using patch-layer structure with a single alpha_PT
# ==============================================================================
def ThreeSEB_PL_singlePT(Tr_K,
                           vza,
                           T_A_K,
                            u,
                            ea,
                           ...
                           massman_profile=[0.0, []],
                            const_L=None):
    '''Three-source model (3SEB) with a single  Priestley-Taylor assumption
    ...
    LAI : float
        Effective Leaf Area Index of primary foliage(m2 m-2).
    LAI_sub: float
        Effective Leaf Area Index of secondary (substrate) foliage(m2 m-2).
    h_C : float
    ...
    [T_AC, L_n_sub, L_nC, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A, delta_Rn, Rn_sub, Rn_C_sub,
      Ln_C_sub, Ln_S,
     Rn_S, iterations] = [np.zeros(Tr_K.shape) + np.NaN for i in range(24)] #add new component outputs
    ...
    ### first try patch layer model --> initial assumption that T_C, T_C_sub and T_S equals T_A
    # First assume that canopy temperature (T_C) equals the minumum of Air or radiometric T
    T_C = np.asarray(np.minimum(Tr_K, T_A_K))
    #T_sub = Tr_K - (f_theta*T_C)/(1-f_theta) #just initial simplification change to take into account vza with calc_T_C (put to the fourth power)...

    flag, T_sub = TSEB.calc_T_S(Tr_K, T_C, f_theta)

    #Assume the substrate canopy temperature equals the minimum off Air or
    # radiometric T --> assume first guess that T_C_sub is mean between T_A and T_sub
    T_C_sub = (T_sub + T_A_K)/2
    #T_C_sub = np.asarray(np.minimum(T_sub, T_A_K))
    ...
    for n_iterations in range(max_iterations):
        ...
        while np.any(LE_S[i] < 0):
            ...
            # adjust b coeficient in KN99 Rs calc for soil conditions
            # for now hard-coded, later adjust to make it an input/parameter
            b_coef = np.ones(Tr_K.shape) * 0.034
            res_params = {'KN_b': b_coef}

            #calculate aerodynamic resistances (using parallet kustas99 approach)
            ...
            L_nC[i], L_n_sub[i] = rad.calc_L_n_Campbell(T_C[i], T_sub[i], L_dn[i], LAI[i], emis_C[i], emis_sub[i], x_LAD=x_LAD[i])#change to emiss_sub
            #L_nC_sub[i], L_nS[i] = rad.calc_L_n_Kustas(T_C_sub[i], T_S[i], L_dn[i], LAI_sub[i], emis_C[i], emis_S[i], x_LAD=x_LAD_sub[i])
            #maybe check for convergance of radiation between layers

            delta_Rn[i] = Sn_C[i] + L_nC[i]
            Rn_sub[i] = Sn_sub[i] + L_n_sub[i]

            # apply beer lambert on substrate
            # _, Ln_C_sub[i], _, Ln_S[i] = calc_Rn_substrate_BeerLambert(Sn_sub[i], L_n_sub[i], LAI_sub[i], np.ones(i.shape)[i], x_LAD_sub[i])
            ...
            # Calculate the canopy and subtrate temperatures using the Priestley
            # Taylor appoach
            ...
            T_C[i] = calc_T_C_Parallel(H_C[i], R_A[i], T_A_K[i], rho[i], R_S[i])
            ...
            # Recalculate soil resistance using new soil and secondary canopy temperature
            _, _, R_S[i] = TSEB.calc_resistances(resistance_form,
                                            {
                                             "R_S": {"u_friction": u_friction[i], "h_C": h_C[i], "d_0": d_0[i],
                                                     ...
                                                     "LAI": LAI_sub[i], "leaf_width": leaf_width[i],"massman_profile": massman_profile,
                                                     "z0_soil": z0_soil[i], "z_u": z_u[i],
            ...
            i = np.logical_and.reduce((LE_S < 0, ~L_converged, flag != F_INVALID))#not exactly sure what this does....

            #get subtrate H flux (maybe dont need this)
            H_sub[i] = rho[i] * c_p[i] * ((T_sub[i] - T_A_K[i]) / (R_S[i]+R_A[i]))
            ...
            H_C_sub[i] = TSEB.calc_H_C_PT(
                Rn_C_sub[i],
                f_g_sub[i],
                T_A_K[i],
                p[i],
                c_p[i],
                alpha_PT_rec[i]) #have the same alpha for now maybe need to seperate alpha between vegetation layers


            # re-calculate resistances with secondary canopy parameters
            # adjust b coeficient in KN99 Rs calc for soil conditions
            # for now hard-coded, later adjust to make it an input/parameter
            b_coef = np.ones(Tr_K.shape) * 0.02
            res_params = {'KN_b': b_coef}

            # Re-calculate soil and bulk resistance for substrate layer
            _, R_x[i], R_S[i] = TSEB.calc_resistances(resistance_form,
                                                      {"R_x": {"u_friction": u_friction[i], "h_C": h_C_sub[i],
                                                               "d_0": d_0_sub[i], "massman_profile": massman_profile,
            ...
            #re-calculate resistances with secondary canopy parameters
            T_C_sub[i] = TSEB.calc_T_C_series(T_sub[i], T_A_K[i], R_A[i], R_x[i], R_S[i],f_theta_sub[i], H_C_sub[i], rho[i], c_p[i])



            # Calculate soil temperature
            flag_t = np.zeros(flag.shape) + F_ALL_FLUXES
            flag_t[i], T_S[i] = TSEB.calc_T_S(T_sub[i], T_C_sub[i], f_theta[i])
            flag[flag_t == F_INVALID] = F_INVALID
            LE_S[flag_t == F_INVALID] = 0
            ...
            H_C_sub[i] = rho[i] * c_p[i] * (T_C_sub[i] - T_AC[i]) / R_x[i]
    ...
    return flag, T_S, T_C, T_C_sub, T_AC, T_sub, F_theta, F_theta_sub, L_n_sub, L_nC, Ln_C_sub, Ln_S,  LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A, u_friction,L, n_iterations

# ==============================================================================
# Function for threeSEB using patch-layer structure with a two(nested) alpha_PT
# ==============================================================================
def ThreeSEB_PL_doublePT(Tr_K,
                           vza,
                           T_A_K,
                            u,
                            ea,
    ...
    Calculates the fluxes using a single observation of
    composite radiometric temperature and with a combined patch-layer model using resistances in parrele for primary foliage and series for substrate foliage.
...
'''
    ...
    for n_iterations in range(max_iterations):
        ...
        # canopy transpiration. Nested loop with different alpha for tree and grass.
        flag[np.logical_and(~L_converged, flag != F_INVALID)] = F_ALL_FLUXES
        LE_S[np.logical_and(~L_converged, flag != F_INVALID)] = -1
        LE_sub[np.logical_and(~L_converged, flag != F_INVALID)] = -1
        #alpha_PT[:]=1.0#tree alpha
        alpha_PT_rec = np.asarray(alpha_PT + 0.1)
        while np.any(LE_sub[i] < 0):
            ...
            #adjust b coeficient in KN99 Rs calc for substrate (grass-soil) conditions
            # for now hard-coded, later adjust to make it an input/parameter
            ...
            # apply beer lambert on substrate (calculate sub-canopy LW and soil LW radiation)
            #_, Ln_C_sub[i], _, Ln_S[i] = calc_Rn_substrate_BeerLambert(Sn_sub[i], L_n_sub[i], LAI_sub[i], np.ones(i.shape)[i], x_LAD_sub[i])
    ...
    return flag, T_S, T_C, T_C_sub, T_AC, L_n_sub, L_nC, Ln_C_sub, Ln_S,  LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_sub, R_x, R_A, u_friction,L, n_iterations, alpha_final, alpha_final_sub
...
def calc_Sn_Campbell(lai, lai_sub, sza, S_dn_dir, S_dn_dif, fvis, fnir, rho_leaf_vis, rho_leaf_vis_sub,
    ...
    # Substrate albedo
    # f_theta_sub = TSEB.calc_F_theta_campbell(sza, LAI_eff_sub, x_LAD_sub)
    # rho_sub = f_theta_sub * rho_soil + (1 - f_theta_sub) * albb_sub
    # Substrate albedo is already the computed albx_sub,
    # where the direct substrate albedo dominates where the overstory gaps are
    # and diffuse albedo otherwise
    #f_theta = TSEB.calc_F_theta_campbell(sza, LAI_eff, x_LAD)
    #try based on shadow cover --> more shadow means diffuse albedo dominates
    #rho_sub = f_theta * albd_sub + (1 - f_theta) * albb_sub
...
def calc_shadow_fraction(sza, hc, hb, wc, f_C, tau):
    ''' Shadow fraction below primary (tree) canopy
        ...
        Estimate shadow fraction below primary (tree) canopy projected onto secondarry (grass vegetation)
...
def calc_subSpectral_Campbell(LAI, sza, hc, hb, wc, f_C, skyl, rho_leaf_vis, tau_leaf_vis, rho_leaf_nir, tau_leaf_nir,
                             rsoilv, rsoiln, x_LAD=1, LAI_eff=None):
    ''' substrate spectral reflectivity characterisitcs

    slightly modified 'calc_Sn_campbel' function
    ...
    hc: float
        Canopy height (m)
    hb: float
        Height where canopy foliage begins (m)
    wc: float
        width to height ratio (-)
    f_C: float
        Fractional cover of primary canopy (tree) (-)
    skyl: float
        Fraction of incoming diffused radiation (-)
    S_dn_dir : float
        Broadband incoming beam shortwave radiation (W m-2).
    S_dn_dif : float
        Broadband incoming diffuse shortwave radiation (W m-2).
    ...
    for angle in range(0, 90, 5):
        akd = rad.calc_K_be_Campbell(angle, x_LAD)  # Eq. 15.4
        taub = np.exp(-akd * LAI)
    ...
    # estimate shade fraction onto grass layer
    f_shaded = calc_shadow_fraction(sza, hc, hb, wc, f_C, taubtv)
    f_sunlit = 1 - f_shaded

    # estimate of diffused fraction arriving to substrate (grass-soil)
    # calculate skyl_grass (proportion of diffused radiation on grass layer
    skyl_grass = f_sunlit * skyl + f_shaded * np.exp(-akb*LAI_eff)

    spectra_Sub_vis = ((1-taubtv)*albbv + taubtv*rsoilv)*(1-skyl_grass) + ((1-taudv)*albdv + taudv*rsoilv)*skyl_grass
    spectra_Sub_nir = ((1-taubtn)*albbn + taubtn*rsoiln)*(1-skyl_grass) + ((1-taudn)*albdn + taudn*rsoiln)*skyl_grass


    return np.asarray(spectra_Sub_vis), np.asarray(spectra_Sub_nir)

def calc_Sn_Campbell_primaryCanopy(LAI, sza, S_dn_dir, S_dn_dif, fvis, fnir, rho_leaf_vis,
    ...
    Added return of the incoming SW radiation in subcanopy (not only Net SW radiation)
    maybe should change soil/ground reflectance constant to 'substrate' reflectance (mix of grass and soil)
    ...
    taud = 0
    for angle in range(0, 90, 5):
        akd = TSEB.rad.calc_K_be_Campbell(angle, x_LAD)  # Eq. 15.4
        taub = np.exp(-akd * LAI)
        taud = taud + taub * np.cos(np.radians(angle)) * \
                      np.sin(np.radians(angle)) * np.radians(5)
    taud = 2.0 * taud
    ...
    # B E A M   C O M P O N E N T S
    # Direct beam extinction coeff (spher. LAD)
    akb = TSEB.rad.calc_K_be_Campbell(sza, x_LAD)  # Eq. 15.4
    # Direct beam canopy reflection coefficients for a deep canopy
    rcpyn = (1.0 - np.sqrt(ameann)) / (1.0 + np.sqrt(ameann))  # Eq 15.7
    rcpyv = (1.0 - np.sqrt(ameanv)) / (1.0 + np.sqrt(ameanv))
    rbcpyn = 2.0 * akb * rcpyn / (akb + 1.0)  # Eq 15.8
    ...
    SW_in_sub_dir = taubtv * S_dn_dir * fvis + taubtn  * S_dn_dir * fnir
    SW_in_sub_dif = taudv * S_dn_dif * fvis + taudn * S_dn_dif * fnir
    return np.asarray(Sn_C), np.asarray(Sn_S), np.asarray(SW_in_sub_dir), np.asarray(SW_in_sub_dif)



def calc_LW_trans_below_Campbell(LAI, LW_in, T_C, emisVeg, x_LAD=1):
    ''' Radiation transmission through a canopy.

    Estimate transmitted Longwave radiation through a canopy using the [Campbell1998]_
    Radiative Transfer Model Kustas et al. (1999).

    Parameters
    ----------
    LAI : float
        Effecive Leaf (Plant) Area Index of primary canopy.
    LW_in: float
        Incoming (downwelling) longwave radiation
    T_C: float
        Above canopy temperature (K)
    emisVeg : float
    ...
    -------
    LW_in_trans : float
        Incoming longwave radiation transferred to the sub-canopy
.
    ...
    # estimate downward LW emission from above canopy
    L_C = emisVeg * met.calc_stephan_boltzmann(T_C)
    # calculate incoming longwave radiation transmitted through the canopy
    LW_in_trans = 0.8 * LW_in + 0.2*(taudl * LW_in + (1-taudl) * L_C)

    return np.asarray(LW_in_trans)
