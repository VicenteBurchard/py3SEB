...
from py3seb.py3seb import raupach_94, calc_Sn_Campbell, ThreeSEB_PT_PT_SEQ, ThreeSEB_PM_PT_SEQ, ThreeSEB_PM_PM_SEQ, \
    ThreeSEB_SW_SEQ
...
outputTxtFieldNames = ['site_ID', 'site_name', 'year', 'hydro_year', 'DOY', 'time', 'SW_in_obs', 'LW_in_obs',
                       'SW_out_obs',
                       ...
                       'H_S', 'flag',
                       'zo', 'd', 'zo_sub', 'd_sub', 'G_model', 'R_s', 'R_sub', 'R_x', 'R_a', 'u_friction', 'L', 'T_TEA']
...
for year in years:
    ...
    for site in sites:
        ...
        output_file = join(outdir_site, 'py3SEB-%s_output_%s_%s_%s_%s.txt' % (py3seb_mode, res_string, G_string, site_name, year))
        ...
        # filter for year in question and some days
        ...
        doy_start = 1
        doy_end = 365
        doy_mask = np.logical_and(metdata['DOY']>doy_start,metdata['DOY']<doy_end)
        temp_mask = np.logical_and(doy_mask, metdata['year_hydro'] >= year)
        # filter for year in question
        metdata = metdata[temp_mask]

        # get dates at half hourly time step
        #dates_str = pd.to_datetime(metdata['Date'].values).strftime('%Y-%m-%d')
        dates_str = pd.to_datetime(metdata['Date'].values, format= 'mixed').strftime('%Y-%m-%d')
        ...
        veg_date_pd = pd.to_datetime(MODIS_LAI['date'].astype('U12'))
        veg_date_doy = veg_date_pd.dayofyear
        start_date = dates_str[0]
        #start_mask = pd.to_datetime(MODIS_LAI['date'].astype('U12')) >= pd.to_datetime(start_date)
        start_mask = np.logical_and(veg_date_doy>doy_start, veg_date_doy<doy_end)
        ...
        sza, saa = TSEB.met.calc_sun_angles(np.ones(metdata['DOY'].shape) * lat,
                                            ...
                                            np.ones(metdata['DOY'].shape) * stdlon, metdata['DOY'],
                                            metdata['hour'].values)
        ...
        sza_capped = np.minimum(sza, 60.0)  # degrees
        Omega = TSEB.CI.calc_omega_Kustas(Omega0, sza_capped, w_C=wC_ratio)
        ...
        vpd = TSEB.met.calc_vapor_pressure(Ta_K) - ea
        p = metdata['P_mb'].values
        u = metdata['u'].values
        u[u < 0] = 0
        T_TEA = metdata['TEA_T'].values
        ...
        [z_0M_sub, d_0_sub] = TSEB.res.calc_roughness(LAI_grass, hC_sub, wC_sub_ratio,
                                                      np.ones(LAI_tree.shape) * landcover_low)
        d_0_sub[d_0_sub < 0] = 0
        ...
        Rn_sw_C, Sn_S, Sn_C_sub = calc_Sn_Campbell(LAI_tree, LAI_grass, sza, Sdn_dir, Sdn_dif, fvis, fnir,
                                                   ...
                                                   x_LAD=x_LAD_C, x_LAD_sub=x_LAD_sub, LAI_eff=LAI_tree_eff,
        ...
        if py3seb_mode == 'PT':
            ...
            [flag_PT_all, T_S, T_C, T_C_sub, T_AC, L_n_sub, L_nC, Ln_C_sub, Ln_S,
             LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G_mod, R_S, R_sub, R_X, R_A, u_friction, L,
             n_iterations] = ThreeSEB_PT_PT_SEQ(LST,
                                         vza,
                                         Ta_K,
                                         u,
                                         ea,
            ...
            [flag_PT_all, T_S, T_C, T_C_sub, T_AC, L_n_sub, L_nC, Ln_C_sub, Ln_S,
             LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G_mod, R_S, R_sub, R_X, R_A, u_friction, L,
             n_iterations] = ThreeSEB_PM_PT_SEQ(LST,
                                         vza,
                                         Ta_K,
                                         u,
                                         ea,
            ...
            # run 3SEB-SW
            [flag_PT_all, T_S, T_C, T_C_sub, T_AC, L_n_sub, L_nC, Ln_C_sub, Ln_S,
             LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G_mod, R_S, R_sub, R_X, R_A, Rss_out, Rst_un_out, Rst_ov_out,
             u_friction, L, n_iterations] = ThreeSEB_SW_SEQ(LST,
                                                            vza,
                                                            Ta_K,
                                                            u,
                                                            ea,
            ...
            print(f'Rss_out:\n{np.nanmean(Rss_out)}')
            print(f'Rst_un\n{np.nanmean(Rst_un_out)}')
            print(f'Rst_ov:\n{np.nanmean(Rst_ov_out)}')
        elif py3seb_mode == 'SW-L':
            ...
            gs[SW_in>100] = gm * leuning_stress(vpd[SW_in>100], d0)

            # Convert stomtata conductance to resistance (s m-1)
            rst_min = 1. / (TSEB.res.molm2s1_2_ms1(Ta_K, p) * gs)
            ...
            [flag_PT_all, T_S, T_C, T_C_sub, T_AC, L_n_sub, L_nC, Ln_C_sub, Ln_S,
             LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G_mod, R_S, R_sub, R_X, R_A, u_friction, L,
             n_iterations] = ThreeSEB_PM_PM_SEQ(LST,
                                         vza,
                                         Ta_K,
                                         u,
                                         ea,
        ...
        validData = np.logical_or.reduce((SW_in < 0, metdata['H'].values == -9999, metdata['SW_in'].values == -9999,
                                          metdata['SW_out'].values == -9999, metdata['LW_out'].values == -9999,
                                          metdata['LW_out'].values == -9999,
                                          np.isnan(metdata['H'].values), np.isnan(metdata['Rn'].values),
                                          metdata['G'].values == -9999,
        ...
        for row in range(LE_PT.size):
            outData = [site, site_name, int(years_actual[row]), year, int(metdata['DOY'].values[row]),
                       metdata['hour'].values[row], SW_in[row],
                       metdata['LW_in'].values[row], metdata['SW_out'].values[row], metdata['LW_out'].values[row],
                       Rn_obs[row],
                       ...
                       wC[row], wC_sub[row], vza, sza[row], Rn_PT[row], Sn_PT[row], Ln_PT[row], Rn_tree[row],
                       ...
                       flag_PT_all[row],
                       z_0M[row], d_0[row], z_0M_sub[row], d_0_sub[row], G_mod[row], R_S[row], R_sub[row], R_X[row],
                       R_A[row],
                       u_friction[row], L[row], T_TEA[row]]
        ...
        noneg = np.logical_and.reduce(
            (H_PT > -50, H_obs > -50, LE_PT > -50, LE_obs > -50, LE_PT < 1500, LE_obs < 1500, H_PT < 1500,
             H_obs < 1500))
        #QC = np.logical_and.reduce((flag_PT_all < 255, metdata['SW_in'].values > 0, Sn_PT > 0))
        #mask = np.logical_and(noneg, QC)
        mask = np.logical_and(flag_PT_all < 255, metdata['SW_in'].values > 0)
        plt.figure()
        ...
        plt.savefig(join(outdir_site, 'py3SEB-%s_FluxPartitioning_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot AE  modeled vs observed scatter plot
        plt.figure()
        plt.scatter(AE_PT[mask], AE_obs[mask], c='orange', marker='.', alpha=0.8, s=3)
        plt.xlim(-100, 700)
        plt.ylim(-100, 700)
        plt.xlabel(r'Modeled $\left(W/m^2\right)$', fontsize=14)
        plt.ylabel(r'Observed $\left(W/m^2\right)$', fontsize=14)
        ...
        plt.savefig(
            join(outdir_site, 'py3SEB-%s_AvailableEnergy_vs_obs_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot AE  modeled vs observed scatter plot
        plt.figure()
        plt.scatter(Sn_PT[mask], Sn_obs[mask], c='orange', marker='.', alpha=0.8, s=3)
        plt.xlim(-100, 1000)
        plt.ylim(-100, 1000)
        plt.xlabel(r'Modeled $\left(W/m^2\right)$', fontsize=14)
        plt.ylabel(r'Observed $\left(W/m^2\right)$', fontsize=14)
        plt.title('Sn ' + str(site_name) + ' ' + str(year))
        plt.plot((-100, 1000), (-100, 1000), 'k-')
        ...
        plt.savefig(
            join(outdir_site, 'py3SEB-%s_Sn_vs_obs_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        '''
        dates = pd.to_datetime(metdata['Date'].values, format= 'mixed')

        import matplotlib.dates as mdates

        mask = metdata['SW_in'].values > 25
        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], H_PT[mask], c='r', marker='.', alpha=0.9, label='H')
        ax.plot(dates[mask], LE_PT[mask], color='b', marker='.', alpha=0.9, label='LE')
        ax.plot(dates[mask], LE_obs[mask], color='royalblue', linestyle='--',marker='s', alpha=0.9, label='LE_obs',)

        ax.plot(dates[mask], Rn_PT[mask], color='orange', marker='.', alpha=0.9, label='Rn')

        # Format major ticks (Dates)
        ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))  # Daily ticks
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # Date format

        # Format minor ticks (Hours)
        ax.xaxis.set_minor_locator(mdates.HourLocator(byhour=range(0, 24, 6)))  # Every 6 hours
        ax.xaxis.set_minor_formatter(mdates.DateFormatter('%H'))  # Hour format
        ax.tick_params(axis='x', which='major', length=20)  # Big ticks
        ax.tick_params(axis='x', which='minor', length=5)  # Small ticks
        ax.set_ylabel('Fluxes (W/m2)')
        plt.legend()
        plt.savefig(
            join(outdir_site, 'py3SEB-%s_TimeSeriesFluxes_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot time seriesH_C + H_C_sub + H_S
        fig, ax = plt.subplots()
        ax.plot(dates[mask], H_C[mask], c='green', marker='.', alpha=0.9, label='H_ov')
        ax.plot(dates[mask], H_C_sub[mask], color='gold', marker='.', alpha=0.9, label='H_un', )
        ax.plot(dates[mask], H_S[mask], color='indianred', marker='.', alpha=0.9, label='H_s')
        # Format major ticks (Dates)
        ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))  # Daily ticks
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # Date format
        ax.set_ylim(0,400)
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_H_Partition_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], LE_C[mask], c='green', marker='.', alpha=0.9, label='LE_ov')
        ax.plot(dates[mask], LE_C_sub[mask], color='gold', marker='.', alpha=0.9, label='LE_un', )
        ax.plot(dates[mask], LE_S[mask], color='indianred', marker='.', alpha=0.9, label='LE_s')
        # Format major ticks (Dates)
        ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))  # Daily ticks
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # Date format
        ax.set_ylim(0,400)
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_LE_Partition_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], Rn_tree[mask], c='green', marker='.', alpha=0.9, label='Rn_ov')
        ax.plot(dates[mask], Rn_C_sub[mask], color='gold', marker='.', alpha=0.9, label='Rn_un', )
        ax.plot(dates[mask], Rn_S[mask], color='indianred', marker='.', alpha=0.9, label='Rn_s')
        # Format major ticks (Dates)
        ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))  # Daily ticks
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # Date format
        ax.set_ylim(0,400)
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_Rn_Partition_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()


        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], L_nC[mask], c='green', marker='.', alpha=0.9, label='Ln_ov')
        ax.plot(dates[mask], Ln_C_sub[mask], color='gold', marker='.', alpha=0.9, label='Ln_un', )
        ax.plot(dates[mask], Ln_S[mask], color='indianred', marker='.', alpha=0.9, label='Ln_s')
        # Format major ticks (Dates)
        ax.xaxis.set_major_locator(mdates.DayLocator(interval=1))  # Daily ticks
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))  # Date format
        ax.set_ylim(-200, 100)
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_Ln_Partition_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], T_C[mask], c='green', marker='.', alpha=0.9, label='$T_{ov}$')
        ax.plot(dates[mask], T_C_sub[mask], color='gold', marker='.', alpha=0.9, label='$T_{un}$', )
        ax.plot(dates[mask], T_S[mask], color='indianred', marker='.', alpha=0.9, label='$T_s$')
        ax.plot(dates[mask], Ta_K[mask], color='k', linestyle='--', alpha=0.9, label='$T_a$')
        ax.plot(dates[mask], T_AC[mask], color='royalblue', linestyle='--', alpha=0.9, label='$T_{AC}$')
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_Temp_Partition_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        if py3seb_mode == 'SW':
            # plot time series
            fig, ax = plt.subplots()
            ax.plot(dates[mask], Rss_out[mask], c='k', marker='.', alpha=0.9, label='Rss')
            ax.plot(dates[mask], Rst_un_out[mask], color='teal', marker='.', alpha=0.9, label='Rst_un')
            ax.plot(dates[mask], Rst_ov_out[mask], color='green', marker='.', alpha=0.9, label='Rst_ov', )
            ...
            plt.savefig(
                join(outdir_site,
                     'py3SEB-%s_Rst_Rss _%s_%s_%s_%s_SEQ.png' % (
                     py3seb_mode, res_string, G_string, year, site_name)))
            plt.close()

        
        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], Rn_sw_C[mask], c='green', marker='.', alpha=0.9, label='Sn_ov')
        ax.plot(dates[mask], sza[mask], color='indianred', marker='.', alpha=0.9, label='sza')
        ax.plot(dates[mask], T_C[mask], color='blue', marker='s', alpha=0.9, label='T_C')
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_Tov_shaded_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()

        # plot time series
        fig, ax = plt.subplots()
        ax.plot(dates[mask], LAI_tree_eff[mask], c='green', marker='.', alpha=0.9, label='LAI_tree_eff')
        ax.plot(dates[mask], LAI_tree[mask], color='indianred', marker='.', alpha=0.9, label='LAI_tree')
        ax.plot(dates[mask], Omega[mask], color='blue', marker='.', alpha=0.9, label='Omega')
        ...
        plt.savefig(
            join(outdir_site,
                 'py3SEB-%s_LAItree_%s_%s_%s_%s_SEQ.png' % (py3seb_mode, res_string, G_string, year, site_name)))
        plt.close()
        '''