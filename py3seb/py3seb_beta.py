...
# ======================================================================================================================
# Function for 3SEB using patch-layer structure with a PM initialization for overstory and PT for understory
# ======================================================================================================================
def ThreeSEB_PM(Tr_K,
                vza,
                T_A_K,
                u,
                ea,
...
'''
    ...
    # Create the output variables
    [T_AC, L_n_sub, L_nC, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S,R_sub, R_x, R_A, delta_Rn, Rn_sub, Rn_C_sub,
      Ln_C_sub, Ln_S, Rn_S, alpha_final_sub, iterations,R_c] = [np.zeros(Tr_K.shape) + np.nan for i in range(27)]
    ...
    for n_iterations in range(max_iterations):
        ...
        alpha_PT_rec_sub = np.asarray(alpha_PT + 0.1)
        alpha_PT_rec = np.asarray(alpha_PT + 0.1)
        #while np.any(np.logical_and.reduce((LE_sub[i] < 0,
        #                                    LE_C[i] > delta_Rn[i],
        #                                    ~T_sub_converged))):
        while np.any(np.logical_or(LE_sub[i] < 0,
                                    H_C[i] < 0)):
            ...
            '''
            # Calculate the canopy and subtrate temperatures using the PT approach
            H_C[i] = calc_H_C_PT(
                delta_Rn[i],
                f_g[i],
                T_A_K[i],
            ...
            #LE_S[i] = -1  # force inner loop to run for all active pixels
            j = i.copy() # start inner loop with consistent i (same pixels)
            while np.any(LE_S[j] < 0):
                # now do series approach to get sub canopy vegetation and soil temperatures
                j = np.logical_and(i, LE_S < 0)
                alpha_PT_rec_sub[j] -= 0.1
        ...
        # ← RECONCILIATION GOES HERE, outside and after the outer while loop
        i = np.logical_and(~L_converged, flag != F_INVALID)
        T_sub_new = f_theta_sub[i] * T_C_sub[i] + (1 - f_theta_sub[i]) * T_S[i]
        T_sub_converged[i] = np.abs(T_sub[i] - T_sub_new) < 0.1
        T_sub[i] = T_sub_new
        H_sub_new = rho[i] * c_p[i] * ((T_sub[i] - T_A_K[i]) / (R_sub[i] + R_A[i]))
        H_sub_converged[i] = np.abs(H_sub[i] - H_sub_new) < 5
        H_sub[i] = H_sub_new

        # re-calculate lw radiaiton with new temperatures
        # Calculate net longwave radiation with current values of T_C, T_C_sub and T_S
        L_nC[i], L_n_sub[i] = rad.calc_L_n_Campbell(T_C[i], T_sub[i], L_dn[i], LAI[i], emis_C[i], emis_sub[i],
                                                    x_LAD=x_LAD[i])

        Ln_C_sub[i], Ln_S[i] = calc_Ln_substrate_Campbell(L_n_sub[i], LAI_sub[i], x_LAD=x_LAD_sub[i],
            emiss_c=emis_sub[i], emiss_s=emis_S[i])
        Rn_C_sub[i] = Sn_C_sub[i] + Ln_C_sub[i]
        Rn_S[i] = Sn_S[i] + Ln_S[i]
        # maybe check for convergance of radiation between layers
        delta_Rn[i] = Sn_C[i] + L_nC[i]
        ...
        # Re-run PM with current r_c to get physically consistent H_C
        gamma_star = psicr[i] * (1. + r_c[i] / R_A[i])
        LE_C[i] = f_g[i] * (delta[i] * delta_Rn[i] / (delta[i] + gamma_star) +
                            (rho[i] * c_p[i] * vpd[i])
                            / (R_A[i] * (delta[i] + gamma_star)))
        H_C[i] = delta_Rn[i] - LE_C[i]  # consistent with PM

        # re-calculate LE as residuals
        LE_C[i] = delta_Rn[i] - H_C[i]
        LE_sub[i] = Rn_sub[i] - H_sub[i] - G[i]

        # Only re-trigger outer loop for valid, non-L-converged, non-T_sub-converged pixels
        recheck = np.logical_and.reduce((np.logical_or(~T_sub_converged,~H_sub_converged ),
                                         ~L_converged,
                                         flag != F_INVALID))
        LE_sub[recheck] = -1
    ...
    for n_iterations in range(max_iterations):
        ...
        for it2 in range(max_it2):
            ...
            if not np.any(LE_S[j] < 0):
                break  # loop2 converged

        i = np.logical_and(~L_converged, flag != F_INVALID)

        # check if there is still negative LE_S
        bad_soil = np.logical_and(i, LE_S < 0)

        if np.any(bad_soil):
            # First try reducing understory alpha
            can_reduce_sub = np.logical_and(bad_soil, alpha_PT_rec_sub > 0)
            alpha_PT_rec_sub[can_reduce_sub] = np.maximum(
                alpha_PT_rec_sub[can_reduce_sub] - 0.1, 0.0)

            # If understory alpha already at 0, reduce overstory alpha
            can_reduce_ov = np.logical_and(bad_soil, alpha_PT_rec_sub == 0)
            can_reduce_ov = np.logical_and(can_reduce_ov, alpha_PT_rec > 0)
            alpha_PT_rec[can_reduce_ov] = np.maximum(
                alpha_PT_rec[can_reduce_ov] - 0.1, 0.0)

            # Force both loops to re-run next n_iterations
            LE_sub[bad_soil] = -1
            LE_S[bad_soil] = -1
...
def ThreeSEB_PM_PT_SEQ(Tr_K,
    ...
    '''Three-source model (3SEB) with PM (overstory) and PT (understory) initialization
...
'''
    ...
    for n_iterations in range(max_iterations):
        ...
        if np.any(bad_soil):
            ...
            # If understory alpha already at 0, reduce overstory alpha
            can_reduce_ov = np.logical_and(bad_soil, alpha_PT_rec_sub == 0)
            can_reduce_ov = np.logical_and(can_reduce_ov, r_c < MAX_RC)
            r_c[can_reduce_ov] = np.minimum(
                r_c[can_reduce_ov] + step_rc, MAX_RC)
...
'''
    ...
    resistance_form = resistance_form[0]
    # Create the output variables
    [T_AC, L_n_sub, L_nC, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S,R_sub, R_x, R_A, R_A_sub, delta_Rn,
     Rn_sub, Rn_C_sub, Ln_C_sub, Ln_S, Rn_S, iterations] = [np.zeros(Tr_K.shape) + np.nan for i in range(26)]
    ...
    c_p = met.calc_c_p(p, ea)  # Heat capacity of air
    z_0H = res.calc_z_0H(z_0M, kB=kB)  # Roughness length for heat transport
    z_0H_sub = res.calc_z_0H(z_0M_sub, kB=kB)  # Roughness length for heat transport
    delta = 10. * met.calc_delta_vapor_pressure(T_A_K)  # slope of saturation water vapour pressure in mb K-1
    lambda_ = met.calc_lambda(T_A_K)  # latent heat of vaporization MJ kg-1
    psicr = met.calc_psicr(c_p, p, lambda_)  # Psicrometric constant (mb K-1)
    es = met.calc_vapor_pressure(T_A_K)  # saturation water vapour pressure in mb
    ...
    for n_iterations in range(max_iterations):
        ...
        step_rc = STEP_RC
        r_c_ov = np.full(Tr_K.shape, r_c_min_ov - step_rc)
        max_it1 = 50
        for it1 in range(max_it1):
            i = np.logical_and.reduce((np.logical_or(LE_sub < 0, H_C < 0),
                                       ~L_converged,
            ...
            step_rc += RELATIVE_INCREASE * step_rc
            # There cannot be negative transpiration from the vegetation
            flag[np.logical_and(i, r_c_ov > MAX_RC)] = F_ZERO_LE
            ...
            # Calculate the canopy and subtrate temperatures using the PM approach
            # Eq. B1 in [Colaizzi2012]_
            gamma_star = psicr[i] * (1. + r_c_ov[i] / R_A[i])
            LE_C[i] = f_g[i] * (delta[i] * delta_Rn[i] / (delta[i] + gamma_star) +
                                (rho[i] * c_p[i] * vpd[i])
                                / (R_A[i] * (delta[i] + gamma_star)))
            H_C[i] = delta_Rn[i] - LE_C[i]
        ...
        i = np.logical_and(~L_converged, flag != F_INVALID)
        LE_S[i] = -1  # explicitly force it2 to run for all active pixels
        # undertory initialization
        step_rc = STEP_RC
        r_c_un = np.full(Tr_K.shape, r_c_min_un - step_rc)
        ...
        for it2 in range(max_it2):
            ...
            # There cannot be negative transpiration from the vegetation
            flag[np.logical_and(i, r_c_un > MAX_RC)] = F_ZERO_LE


            R_A_sub[j], R_x[j], R_S[j] = TSEB.calc_resistances(resistance_form,
                                                           {
                                                            "R_A": {"z_T": z_T[j], "u_friction": u_friction[j],
                                                                        "L": L[j],
                                                                        "d_0": d_0_sub[j], "z_0H": z_0H_sub[j]},
                                                            "R_x": {"u_friction": u_friction[j], "h_C": h_C_sub[j],
                                                                    "d_0": d_0_sub[j],
                                                                    "z_0M": z_0M_sub[j], "L": L[j], "F": F_sub[j],
                                                                    "LAI": LAI_sub[j],
            ...
            # Calculate the canopy and subtrate temperatures using the PM approach
            # Eq. B1 in [Colaizzi2012]_
            gamma_star = psicr[j] * (1. + r_c_un[j] / R_A_sub[j])
            LE_C_sub[j] = f_g_sub[j] * (delta[j] * Rn_C_sub[j] / (delta[j] + gamma_star) +
                                (rho[j] * c_p[j] * vpd[j])
                                / (R_A_sub[j] * (delta[j] + gamma_star)))
            H_C_sub[j] = Rn_C_sub[j] - LE_C_sub[j]
        ...
        if np.any(bad_soil):
            # First try increasing understory r_c_un
            can_reduce_sub = np.logical_and(bad_soil, r_c_un < MAX_RC)
            r_c_un[can_reduce_sub] = np.minimum(
                r_c_un[can_reduce_sub] + step_rc, MAX_RC)

            # If understory r_c_un is already at max, increase overstory r_c_ov
            can_reduce_ov = np.logical_and(bad_soil, r_c_un == MAX_RC)
            can_reduce_ov = np.logical_and(can_reduce_ov, r_c_ov < MAX_RC)
            r_c_ov[can_reduce_ov] = np.minimum(
                r_c_ov[can_reduce_ov] + step_rc, MAX_RC)
...
def ThreeSEB_2T(T_C,
                ...
                massman_profile=[0.0, []],
                const_L=None):
    '''Three-source model (3SEB) using component canopy and substrate (veg+soil) temperature

    Calculates the fluxes using two (vegetation and substrate) temperatures measured or derived previously
    from  radiometric temperature.
    Combined patch-layer model using resistances in parellel for primary foliage and series for substrate foliage.
    ...
    T_C : float
        canopy temperature (Kelvin).
    T_sub : float
        substrate (veg+soil) temperature (Kelvin).
    vza : float
        View Zenith Angle (degrees).
    T_A_K : float
        Air temperature (Kelvin).
...
'''
    ...
    resistance_form = resistance_form[0]
    # Create the output variables
    [L_n_sub, Ln_C, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_sub, R_x_un, R_x_ov, R_A,
     delta_Rn, Rn_C_sub, Ln_C_sub, Ln_S, Rn_S, iterations] = [np.zeros(T_C.shape) + np.nan for i in range(24)]

    Sn_sub = Sn_S + Sn_C_sub
    # iteration of the Monin-Obukhov length
    if const_L is None:
        # Initially assume stable atmospheric conditions and set variables for
        L = np.asarray(np.zeros(T_C.shape) + np.inf)
        max_iterations = ITERATIONS
    else:  # We force Monin-Obukhov lenght to the provided array/value
        L = np.asarray(np.ones(T_C.shape) * const_L)
        max_iterations = 1  # No iteration
    ...
    Ln_C, Ln_sub = rad.calc_L_n_Campbell(T_C, T_sub, L_dn, LAI, emis_C, emis_sub, x_LAD=x_LAD)

    # apply Cambpell longwave transmittance on substrate
    # (calculate sub-canopy LW and soil LW radiation)
    Ln_C_sub, Ln_S = calc_Ln_substrate_Campbell(Ln_sub,
                                               LAI_sub,
                                               x_LAD=x_LAD_sub,
                                               emiss_c=emis_sub,
                                               emiss_s=emis_S)

    # net radiation of main canopy
    Rn_C = Sn_C + Ln_C
    ...
    u_friction = np.asarray(np.maximum(u_friction_min, u_friction))
    L_queue = deque([np.array(L)], 6)
    L_converged = np.asarray(np.zeros(T_C.shape)).astype(bool)
    L_diff_max = np.inf

    # First assume that canopy temperature equals the minumum of Air or
    # radiometric T
    # Assume first guess that T_C_sub is mean between T_A and T_sub
    T_C_sub = np.asarray(np.minimum(T_sub, T_C), dtype=np.float32)
    #T_C_sub = (T_sub + T_C) / 2
    flag, T_S = TSEB.calc_T_S(T_sub, T_C_sub, f_theta_sub)
    T_AC = T_A_K.copy()
    ...
    for n_iterations in range(max_iterations):
        ...
        # Calculate aerodynamic resistances for overstory layer
        R_A[i], R_x_ov[i], R_sub[i] = TSEB.calc_resistances(
        ...
        # Calculate aerodynamic resistances for understory layer
        _, R_x_un[i], R_S[i] = TSEB.calc_resistances(
                resistance_form,
                        {"R_x": {"u_friction": u_friction[i], "h_C": h_C_sub[i],
                                          "d_0": d_0_sub[i],
                                          "z_0M": z_0M_sub[i], "L": L[i], "F": F_sub[i], "LAI": LAI_sub[i],
                                  ...
                                  "R_S": {"u_friction": u_friction[i], "h_C": h_C_sub[i],
                                          ...
                                          "z0_soil": z0_soil[i], "z_u": z_u[i],
                                          "deltaT": T_S[i] - T_C_sub[i], 'u': u[i], 'rho': rho[i],
        ...
        # Compute air temperature at the canopy interface (incorporate extra source)
        ...
        #T_AC[i] = ((T_A_K[i] / R_A[i] + T_sub[i] / R_sub[i] + T_C[i] / R_x_ov[i])
        #           / (1 / R_A[i] + 1 / R_sub[i] + 1/R_x_ov[i]))

        T_AC = np.asarray(np.maximum(1e-3, T_AC))

        # calculation H_C from equation 14 of Norman1995
        # (using series approach also tree canopy)
        # TODO: check if correct formulation
        H_C[i] = rho[i] * c_p[i] * (T_C[i] - T_AC[i]) / R_x_ov[i]

        # Compute air temperature at the canopy interface
        #T_AC[i] = ((T_A_K[i] / R_A[i] + T_S[i] / R_S[i] + T_C_sub[i] / R_x[i])
        #           / (1 / R_A[i] + 1 / R_S[i] + 1 / R_x[i]))

        # T_AC = np.asarray(np.maximum(1e-3, T_AC))

        # calculation H_C from equation 14 of Norman1995
        # (using parallel approach for tree canopy)
        #H_C[i] = rho[i] * c_p[i] * (T_C[i] - T_A_K[i]) / R_A[i]
        ...
        # loop to iterativelly reduce alpha_PT in case latent heat flux
        # from the soil or substrate is negative. The initial assumption is of potential
        ...
        while np.any(LE_S[i] < 0):
            ...
            # Ln_C_sub[i], Ln_S[i] = rad.calc_L_n_Campbell(
            #    T_C_sub[i], T_S[i], L_dn[i], LAI[i], emis_C[i], emis_S[i], x_LAD=x_LAD[i])
            # Rn_C_sub[i] = Sn_C_sub[i] + Ln_C_sub[i]
            # Rn_S[i] = Sn_S[i] + Ln_S[i]

            # Calculate the canopy and soil temperatures using the Priestley
            # Taylor appoach
            H_C_sub[i] = calc_H_C_PT(
                Rn_C_sub[i],
                f_g_sub[i],
                T_A_K[i],
                p[i],
            ...
            T_C_sub[i] = TSEB.calc_T_C_series(T_sub[i], T_A_K[i], R_A[i], R_x_un[i], R_S[i],
                                              f_theta_sub[i], H_C_sub[i], rho[i], c_p[i])
            ...
            _, _, R_S[i] = TSEB.calc_resistances(resistance_form,
                                                 {"R_S": {"u_friction": u_friction[i], "h_C": h_C_sub[i],
                                                          ...
                                                          "massman_profile": massman_profile,
                                                          "z0_soil": z0_soil[i], "z_u": z_u[i],
                                                          "deltaT": T_S[i] - T_C_sub[i], 'u': u[i],
                                                          'rho': rho[i],
                                                          "c_p": c_p[i], "f_cover": f_c_sub[i],
                                                          "w_C": w_C_sub[i],
                                                          "res_params": {k: res_params[k][i] for k in
                                                                         res_params.keys()}}
                                                  }
            ...
            # and the energy at the soil should be conserved.
            # See end of appendix A1 in Guzinski et al. (2015).
            noT = np.logical_and(i, LE_C_sub == 0)
            H_S[noT] = np.minimum(H_S[noT], Rn_S[noT] - G[noT])
            G[noT] = np.maximum(G[noT], Rn_S[noT] - H_S[noT])
            LE_S[noT] = 0
    ...
    return (flag, T_S, T_C_sub, T_AC, L_n_sub, Ln_C, Ln_C_sub, Ln_S, LE_C, H_C,
            LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x_un,R_x_ov, R_A, u_friction, L, n_iterations)
...
def ThreeSEB_3T(T_C,
    ...
    ----------
    T_C : float
        Primary or overstory canopy temperature (Kelvin).
    T_C_sub : float
        Secondary or nderstory canopy temperature (Kelvin).
    T_S : float
        soil temperature (Kelvin).
    T_A_K : float
        Air temperature (Kelvin).
...
'''
    ...
    resistance_form = resistance_form[0]
    # Create the output variables
    [flag, H_C, H_C_sub, H_S,G, R_S, R_x_ov, R_x_un, R_A, iterations] = [np.zeros(T_C.shape) + np.nan for i in range(10)]
    ...
    # And the net longwave radiation
    Ln_C, Ln_sub = rad.calc_L_n_Campbell(T_C, (T_C_sub + T_S)/2, L_dn, LAI, emis_C, emis_S, x_LAD=x_LAD)
    ...
    for n_iterations in range(max_iterations):
        ...
        # Calculate aerodynamic resistances for overstory layer
        R_A[i], R_x_ov[i], _ = TSEB.calc_resistances(
                resistance_form, {"R_A": {"z_T": z_T[i], "u_friction": u_friction[i], "L": L[i],
                                          "d_0": d_0[i], "z_0H": z_0H[i]},
                                  "R_x": {"u_friction": u_friction[i], "h_C": h_C[i],
                                          "d_0": d_0[i],
        ...
        '''
        # TODO: check if correct formulation
        # Calculate sub-canopy sensible heat flux using series appraoch (Norman et al 1995)
        H_C_sub[i] = rho[i] * c_p[i] * (T_C_sub[i] - T_AC[i]) / R_x_un[i]
        # Assume no condensation in the canopy (LE_C<0)
        noC = np.logical_and(i, H_C_sub > Rn_C_sub)
        H_C_sub[noC] = Rn_C_sub[noC]
        flag[noC] = F_ZERO_LE_C
        # Assume no thermal inversion in the canopy
        noI = np.logical_and.reduce(
            (i,
    ...
    return flag, T_AC, Ln_C, Ln_C_sub, Ln_S,  LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x_un,R_x_ov, R_A, u_friction,L, n_iterations
...
def calc_shadow_fraction(sza, hc, hb, wc, f_C, tau):
    ...
    # Convert SZA to radians and cap to avoid tan blowing up
    sza_rad = np.radians(np.minimum(sza, 80.0))  # cap at 80 degrees

    # Shadown length
    shadow_length = (hc - hb) * np.tan(sza_rad)
...
def calc_T_C(T_R, T_sub, f_theta):
    '''Estimates canopy temperature from the directional LST and substrate temp.
    ...
    T_R, T_sub, f_theta = map(np.asarray, (T_R, T_sub, f_theta))
    T_temp = T_R**4 - ((1-f_theta) * T_sub**4)
    T_C = np.zeros(T_R.shape, np.float32)
    flag = np.zeros(T_R.shape, np.int32) + F_ALL_FLUXES

    # Succesfull inversion
    T_C[T_temp >= 0] = (T_temp[T_temp >= 0]
                        / (f_theta[T_temp >= 0]))**0.25

    # Unsuccesfull inversion
    T_C[np.logical_or(T_temp < 0, np.isnan(T_temp))] = 1e-6
    flag[np.logical_or(T_temp < 0, np.isnan(T_temp))] = F_INVALID

    return np.asarray(flag, dtype=np.int32), np.asarray(T_C, dtype=np.float32)
